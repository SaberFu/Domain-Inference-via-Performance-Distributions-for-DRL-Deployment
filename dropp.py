...
class DROPP(object):
	...
	optimize_dynamics_distribution(...)
	    Starts the main DROPP optimization problem

	set_offline_dataset(...)
		Sets the offline dataset of transitions used for running DROPP
	...
	def set_offline_dataset(self, T, indexes=None, n=None, sparse_mode=False):
		"""Sets the offline state transitions used for running DROPP.
		...
		indexes : list, optional
			...
			running DROPP. If None, transitions are automatically selected
		...
		sparse_mode : boolean, optional
			if True, DROPP is run on random sparse transitions, rather than
